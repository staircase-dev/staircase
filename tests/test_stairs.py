...
def test_one_finite_interval(init_value, added_interval):
    ...
    assert _compare_iterables(
        int_seq.step_changes(), (start, end)
...
def test_two_adjacent_finite_interval_same_value(init_value, endpoints, value):
    ...
    assert _compare_iterables(
        int_seq.step_changes(), (point1, point3)
    ), "Finite endpoints are not what is expected"
    assert (
        int_seq(float("-inf")) == init_value
    ), "Adding finite interval should not change initial value"
...
def test_two_adjacent_finite_interval_different_value(
    ...
    assert _compare_iterables(
        int_seq.step_changes(), (point1, point2, point3)
    ), "Finite endpoints are not what is expected"
    assert (
        int_seq(float("-inf")) == init_value
    ), "Adding finite interval should not change initial value"
...
@pytest.mark.parametrize(
    ...
    [
        ({}, -1.75,),
        ({"lower": 1, "upper": 6}, 0.25,),
        ({"lower": 1, "upper": 6, "lower_how": "left"}, -1.75,),
        ({"lower": 1, "upper": 6, "upper_how": "right"}, -0.5,),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ({}, 2.75,),
        ({"lower": -4, "upper": 1}, -1.75,),
        ({"lower": -4, "upper": 1, "lower_how": "left"}, 0.0,),
        ({"lower": -4, "upper": 1, "upper_how": "right"}, 0.25,),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"lower": -4, "upper": 10}, {-1.75, -0.5, 0.25, 2.0, 2.75},),
        ({"lower": 1, "upper": 6}, {0.25, 2.0, 2.75}),
        ({"lower": 1, "upper": 6, "lower_how": "left"}, {-1.75, 0.25, 2.0, 2.75},),
        ({"lower": 1, "upper": 6, "upper_how": "right"}, {-0.5, 0.25, 2.0, 2.75},),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ([-4, -2, 1, 3], {}, [-1.75, -1.75, 0.25, 2.75],),
        ([-4, -2, 1, 3], {"how": "right"}, [-1.75, -1.75, 0.25, 2.75],),
        ([-4, -2, 1, 3], {"how": "left"}, [0.0, -1.75, -1.75, 0.25],),
    ],
)

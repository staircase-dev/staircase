...
Major enhancements have also been added, namely :ref:`masking <user_guide.masking>`, :ref:`slicing <user_guide.slicing>` and the implicit handling of dates.  While much of the API has remained the same, or similar, there are many backwards incompatible changes and deprecations, which are detailed below.

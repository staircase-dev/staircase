...
def _min_pair(stairs1, stairs2):
    ...
    """
    assert isinstance(stairs1, sc.Stairs) and isinstance(stairs2, sc.Stairs), "Arguments to min must be both of type Stairs."
    ...
    new_instance = sc.Stairs(dict(zip(new_instance._keys(), deltas)), use_dates=stairs1.use_dates or stairs2.use_dates, tz=stairs1.tz)
...
def _max_pair(stairs1, stairs2):
    ...
    """
    assert isinstance(stairs1, sc.Stairs) and isinstance(stairs2, sc.Stairs), "Arguments to max must be both of type Stairs."
    assert stairs1.tz == stairs2.tz
    new_instance = stairs1-stairs2
    cumulative = new_instance._cumulative()
    for key,value in cumulative.items():

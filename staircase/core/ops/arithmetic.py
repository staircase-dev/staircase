...
def _mul_or_div(self, other, op):
    ...
    multiplied_cumulative_values = op(a._cumulative().values(), b._cumulative().values())
    new_instance = sc.Stairs.from_cumulative(dict(zip(a._keys(), multiplied_cumulative_values)), use_dates=self.use_dates, tz=self.tz)
    new_instance._reduce()
    return new_instance

...
from staircase.core.tools.datetimes import check_binop_timezones, _convert_date_to_float, _convert_float_to_date
...
def var(self, lower=float('-inf'), upper=float('inf')):
    ...
    Stairs.std
    """
    percentile_minus_mean = (percentile_stairs(self, lower, upper) - self.mean(lower, upper))._cumulative()
    return (
